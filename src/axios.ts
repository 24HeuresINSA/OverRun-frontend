...
axios.interceptors.response.use(
    ...
    async (err) => {
        ...
        if (err.response.status === 403) {
            ...
            if ((await response).status === 200 && (await response).data.accessToken && (await response).data.refreshToken) {
                console.log("Hello")
                store.commit(MutationTypes.SET_ACCESS_TOKEN, (await response).data.accessToken); 
